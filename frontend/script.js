document.addEventListener('DOMContentLoaded', () => {
  const API_BASE = 'http://127.0.0.1:8000/api'; // adjust if backend is on another origin
  ...
  const openMedAIFromHero = document.getElementById('openMedAIFromHero');
  const openPortalFromHero = document.getElementById('openPortalFromHero');

  if (openMedAIFromHero) {
    openMedAIFromHero.addEventListener('click', () => {
      openPage('MedAI');
      const input = document.getElementById('MedAIInput');
  ...
  // ---------- MedAI ----------
  const MedAIInput = document.getElementById('MedAIInput');
  const MedAITextBtn = document.getElementById('MedAITextBtn');
  const MedAIVoiceBtn = document.getElementById('MedAIVoiceBtn');
  const MedAIStatus = document.getElementById('MedAIStatus');
  const MedAIResponse = document.getElementById('MedAIResponse');
  ...
  async function sendMedAI(text) {
    if (!text) return;
    MedAIStatus.textContent = 'Analyzing symptoms...';
    MedAIResponse.hidden = true;

    try {
      const res = await fetch(`${API_BASE}/MedAI/triage`, {
        method: 'POST',
      ...
      MedAIStatus.textContent = '';
      MedAIResponse.hidden = false;
    } catch (err) {
      // demo fallback
  ...
  MedAITextBtn.addEventListener('click', () => {
    const text = MedAIInput.value.trim();
    sendMedAI(text);
  });
  ...
  if (!SpeechRecognition) {
    MedAIVoiceBtn.disabled = true;
    MedAIVoiceBtn.textContent = '🎤 Not supported';
  } else {
    ...
    recognition.onstart = () => {
      recognizing = true;
      MedAIStatus.textContent = 'Listening... speak your symptoms.';
    };
    recognition.onend = () => {
      recognizing = false;
      MedAIStatus.textContent = '';
    ...
    recognition.onerror = () => {
      recognizing = false;
      MedAIStatus.textContent = 'Could not capture audio. Try again or type your symptoms.';
    };
    recognition.onresult = (event) => {
      const transcript = event.results[0][0].transcript;
      MedAIInput.value = transcript;
      sendMedAI(transcript);
    };

    MedAIVoiceBtn.addEventListener('click', () => {
      if (recognizing) {
        recognition.stop();
      } else {
        recognition.start();
